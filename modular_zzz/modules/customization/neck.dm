/datum/sprite_accessory/fluff/moth/acradors
	name = "Neckfluff (Acradors 1)"
	icon_state = "neckfluff_acradors_1"
	icon = 'modular_zzz/icons/mob/sprite_accessory/neck_large.dmi'
	color_src = USE_ONE_COLOR
	relevent_layers = list(BODY_ADJ_LAYER, BODY_FRONT_LAYER)

/datum/sprite_accessory/fluff/moth/acradors/second
	name = "Neckfluff (Acradors 2)"
	icon_state = "neckfluff_acradors_2"

/datum/sprite_accessory/fluff/moth/acradors/third
	name = "Neckfluff (Acradors 3)"
	icon_state = "neckfluff_acradors_3"

/datum/sprite_accessory/fluff/moth/acradors/fourth
	name = "Neckfluff (Acradors 4)"
	icon_state = "neckfluff_acradors_4"

/datum/sprite_accessory/fluff/moth/acradors/fifth
	name = "Neckfluff (Acradors 5)"
	icon_state = "neckfluff_acradors_5"

/datum/sprite_accessory/fluff/moth/acradors/sixth
	name = "Neckfluff (Acradors 6)"
	icon_state = "neckfluff_acradors_6"
